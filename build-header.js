...
https.get(imageUrl, (res) => {
    ...
    res.on('end', () => {
        ...
        const stars = Array.from({ length: 180 }).map((_, i) => {
            ...
            const color = Math.random() > 0.6 ? '#C084FC' : (Math.random() > 0.3 ? '#818CF8' : '#FFFFFF');
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <!-- Deep Space Void & Nebula Gradients -->
    <radialGradient id="nebula-purple-core" cx="50%" cy="50%" r="55%">
      <stop offset="0%" stop-color="#8B5CF6" stop-opacity="0.55" />
      <stop offset="25%" stop-color="#6D28D9" stop-opacity="0.4" />
      <stop offset="55%" stop-color="#4C1D95" stop-opacity="0.2" />
      <stop offset="80%" stop-color="#1E1B4B" stop-opacity="0.08" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>

    <radialGradient id="nebula-blue-glow" cx="62%" cy="46%" r="40%">
      <stop offset="0%" stop-color="#3B82F6" stop-opacity="0.35" />
      <stop offset="50%" stop-color="#1D4ED8" stop-opacity="0.15" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>

    <radialGradient id="nebula-pink-burst" cx="38%" cy="52%" r="45%">
      <stop offset="0%" stop-color="#D946EF" stop-opacity="0.35" />
      <stop offset="50%" stop-color="#A21CAF" stop-opacity="0.15" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>
    ...
    <style>
      ...
      .photo-glow {
        filter: drop-shadow(0 0 60px rgba(139, 92, 246, 0.45)) drop-shadow(0 25px 50px rgba(0, 0, 0, 0.9));
      }
  ...
  <g transform="translate(${width / 2}, 420)" class="nebula-pulse" style="transform-origin: center;">
    <ellipse cx="0" cy="0" rx="650" ry="320" fill="url(#nebula-purple-core)" />
    <ellipse cx="120" cy="-30" rx="420" ry="220" fill="url(#nebula-blue-glow)" />
    <ellipse cx="-120" cy="40" rx="460" ry="240" fill="url(#nebula-pink-burst)" />
  </g>

  <!-- Top Metadata Bar -->
  <g transform="translate(${width / 2}, 55)">
    <line x1="-360" y1="-5" x2="-220" y2="-5" stroke="#475569" stroke-width="1" />
    <text x="0" y="0" class="top-meta">SOFTWARE ENGINEER (MATHIYA)</text>
    <line x1="220" y1="-5" x2="360" y2="-5" stroke="#475569" stroke-width="1" />
  </g>
  ...
  <g class="photo-glow">
    <image href="${imageBase64}" x="${width / 2 - 240}" y="240" width="480" height="520" preserveAspectRatio="xMidYMid slice" />
  </g>

  <!-- Layer 3: Solid "ANGIRASA" (Foreground Bold Typography) -->
  ...
  <text x="${width / 2}" y="855" class="bottom-meta">SOFTWARE ENGINEER &amp; AI SYSTEMS ARCHITECT</text>
        ...
        console.log("Created header.svg successfully matching reference image!");
    });
});
