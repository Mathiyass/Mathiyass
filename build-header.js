...
https.get(imageUrl, (res) => {
    ...
    res.on('end', () => {
        ...
        // Generate fine cosmic star particles
        const stars = Array.from({ length: 260 }).map((_, i) => {
            const x = Math.random() * width;
            const y = Math.random() * height;
            const r = Math.random() * 1.5 + 0.3;
            const op = (Math.random() * 0.7 + 0.15).toFixed(2);
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <radialGradient id="nebula-purple-core" cx="50%" cy="50%" r="55%">
      ...
      <stop offset="55%" stop-color="#4C1D95" stop-opacity="0.22" />
      <stop offset="80%" stop-color="#1E1B4B" stop-opacity="0.08" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>

    <radialGradient id="nebula-blue-glow" cx="62%" cy="46%" r="42%">
      <stop offset="0%" stop-color="#3B82F6" stop-opacity="0.4" />
      <stop offset="50%" stop-color="#1D4ED8" stop-opacity="0.18" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>

    <radialGradient id="nebula-pink-burst" cx="38%" cy="52%" r="45%">
      <stop offset="0%" stop-color="#D946EF" stop-opacity="0.4" />
      <stop offset="50%" stop-color="#A21CAF" stop-opacity="0.18" />
      <stop offset="100%" stop-color="transparent" stop-opacity="0" />
    </radialGradient>
    ...
    <style>
      ...
      .photo-glow {
        filter: drop-shadow(0 0 70px rgba(147, 51, 234, 0.55)) drop-shadow(0 25px 50px rgba(0, 0, 0, 0.95));
      }
  ...
  <text x="${width / 2}" y="855" class="bottom-meta">SOFTWARE ENGINEER &amp; AI SYSTEMS ARCHITECT</text>
        ...
        console.log("Updated header.svg to 100% clean cinematic reference!");
    });
});
